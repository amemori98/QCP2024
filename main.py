        ...
        def __init__(self, array, id=""):
                ...
                if isinstance(array, Sparse):
                        ...
                        for i in range(len(array.elements)):
                                matrix[array.indices[i, 0],
                                             array.indices[i, 1]] = array.elements[i]
                        self.matrix = matrix
...
class Sparse(Matrix):
        ...
        def __init__(self, array, id=""):
                ...
                assert isinstance(
                        array,
                        (np.ndarray, SparseRep,
                         Dense)), "Can only convert an array, SparseRep or Matrix classes"
        ...
        def __mod__(self, other):
                ...
                for i in range(len(self.elements)):
                        for j in range(len(other.elements)):
                                ...
                                indices.append([
                                        self.indices[i, 0] * other.rows + other.indices[j, 0],
                ...
                return Sparse(
                        SparseRep(kronecker, indices, self.rows * other.rows,
...
def CNOT(qubit_count, control_list, target_list, id=""):
        """
                ...
                """
        assert qubit_count >= (
                len(control_list) + len(target_list)
        ), "Number of qubits must be greater than or equal to the number of control and target qubits"
        assert isinstance(qubit_count, int), "Qubit count must be an integer"
        assert isinstance(control_list,
                                            list), "Control qubits must be provided as a list"
        assert isinstance(target_list,
                                            list), "Target qubits must be provided as a list"
        for control in control_list:
                assert isinstance(control,
                                                    int), "One or more control qubits is not an integer"
        for target in target_list:
                assert isinstance(target,
                                                    int), "One or more target qubits is not an integer"
        assert len(control_list) == len(
                set(control_list)), "Control qubits must be unique"
        assert len(target_list) == len(
                set(target_list)), "Target qubits must be unique"
...
class programmer(object):
        ...
        def add_step(self, gates, step_number=-1):
                ...
                assert step_number >= 0 and step_number <= len(
                        self.steps), "Step number must be within the already defined steps"
                assert isinstance(step_number, int), "Step number must be an integer"
                assert isinstance(
                        gates, list
                ), "Gates must be provided as a list of single qubits or a list with a single n-qubit gate"
                assert len(gates) == 1 or len(
                        gates
                ) == self.qubit_count, "Number of gates must be equal to the number of qubits in the register, unless it is an n-qubit gate"
        ...
        def get_matrix(self):
                ...
                if self.compiled:
                        ...
                        raise Exception(
                                "Circuit has not been compiled. Please compile the circuit before getting the matrix representation."
                        )
