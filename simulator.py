            ...
            for j in range(self.cols):
                for k in range(other.rows):
                    for l in range(other.cols):
                        tensor[i*self.rows+k,j*self.cols+l] = self.matrix[i,j]*other.matrix[k,l]
...
class SparseMatrix(object):
    def __init__(self, array):
        ...
        """
        assert isinstance(array,np.ndarray) or isinstance(array,SparseRep), "Argument is not an array or list"
